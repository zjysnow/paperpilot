...
export async function detectSkillIntent(
  ...
  try {
    ...
    Zotero.debug?.(
      `[paperpilotfor-zotero] Skill classifier LLM call failed, falling back to regex: ${
  ...
  if (parsed === null) {
    Zotero.debug?.(
      `[paperpilotfor-zotero] Skill classifier returned malformed JSON, falling back to regex. Raw: ${raw.slice(0, 200)}`,
    );
    return regexFallback(skills, request);
  }
  return parsed;
