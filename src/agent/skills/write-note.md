...
- **Footer is mandatory on every note** (paper or general, Zotero or file-based). End the note with a horizontal rule followed by `Written by Paper Pilot.` on its own line, exactly as shown in the templates. For HTML Zotero notes, use `<hr/><p>Written by Paper Pilot.</p>`.
...
8. The note ends with the footer `---` then a blank line then `Written by Paper Pilot.` (or the HTML equivalent for Zotero HTML notes).
...
- **Every note ends with the footer** `---\n\nWritten by Paper Pilot.` — no exceptions, no omissions, regardless of destination or format.
