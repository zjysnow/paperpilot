...
function buildUI(body: Element, item?: Zotero.Item | null) {
  ...
  // Main container
  const container = createElement(doc, "div", "paperpilot-panel", {
    id: "paperpilot-main",
  });
  ...
  const makeSlashItem = (id: string, title: string, desc: string) => {
    ...
    const titleEl = createElement(
      doc,
      "span",
      "paperpilot-action-picker-title",
      {
        textContent: title,
      },
    );
  ...
  const paperPreview = createElement(
    doc,
    "div",
    "paperpilot-paper-context-inline",
    {
  ...
  reasoningBtn.title = "Reasoning controls are not supported by Ollama yet.";
  reasoningBtn.setAttribute(
    "aria-label",
    "Reasoning controls are not supported by Ollama yet.",
  );
