...
function getMessageAnchorForElement(element: Element): {
  ...
  const wrapper = closestElement(element, ".paperpilot-message-wrapper");
...
function buildQuoteAnchor(
  ...
  const quoteCard = closestElement(element, ".paperpilot-quote-card") || element;
...
function findBestVisibleChatAnchor(
  ...
  const quoteCandidates = [
    ...queryElements(chatBox, ".paperpilot-quote-card"),
  ...
  for (const candidate of quoteCandidates) {
    const quoteCard = closestElement(candidate, ".paperpilot-quote-card") || candidate;
  ...
  for (const candidate of queryElements(chatBox, ".paperpilot-message-wrapper")) {
...
function findChatAnchorForElement(
  ...
  const messageElement = closestElement(element, ".paperpilot-message-wrapper");
  if (messageElement) {
...
function findMessageWrapperForAnchor(
  ...
  return (
    queryElements(chatBox, ".paperpilot-message-wrapper").find(
      (element) =>
...
function findQuoteElementForAnchor(
  ...
  if (anchor.quoteCitationId) {
    for (const root of roots) {
      const match = queryElements(root, ".paperpilot-quote-card").find(
        (element) =>
  ...
  if (anchor.citationSyncKey) {
    for (const root of roots) {
      ...
      if (match) return closestElement(match, ".paperpilot-quote-card") || match;
    }
  }
  return null;
}
