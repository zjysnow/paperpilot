...
export function createActionLayoutController(
  ...
  const setActionButtonLabel = (
    ...
    button.classList.toggle("paperpilot-action-icon-only", mode === "icon");
  };
  ...
  const setPanelActionLayoutMode = (mode: ActionLayoutMode) => {
    if (panelRoot.dataset.paperpilotActionLayoutMode !== mode) {
      panelRoot.dataset.paperpilotActionLayoutMode = mode;
    }
  };
  ...
  const applyResponsiveActionButtonsLayout = () => {
    ...
    const applyMeasurementBaseline = () => {
      ...
      modelBtn.classList.toggle("paperpilot-model-btn-collapsed", false);
      modelSlot?.classList.toggle("paperpilot-model-dropdown-collapsed", false);
      modelBtn.classList.toggle("paperpilot-model-btn-wrap-2line", false);
    ...
    const applyState = (state: ActionRevealState) => {
      ...
      modelBtn.classList.toggle(
        "paperpilot-model-btn-wrap-2line",
