export function clearCommandRowState(params: {
  ...
  const row = params.body.querySelector("#paperpilotcommand-row");
  if (row) {
    row.removeAttribute("data-active");
    row.classList.remove("paperpilotcommand-row--skill");
  }
...
export function activateCommandRowState(params: {
  ...
  const row = params.body.querySelector("#paperpilotcommand-row");
  const badgeEl = params.body.querySelector("#paperpilotcommand-row-badge");
  if (!row || !badgeEl) return null;
  badgeEl.textContent = params.label;
  if (params.kind === "skill") {
    row.classList.add("paperpilotcommand-row--skill");
  } else {
    row.classList.remove("paperpilotcommand-row--skill");
  }
