...
export function attachFloatingMenuInteractionController(
  ...
  const dismissPaperPickerOnOutsidePointerDown = (event: PointerEvent) => {
    ...
    const inputBox = body.querySelector("#paperpilot-input") as HTMLElement | null;
  ...
  const dismissShortcutMenuOnEscape = (event: KeyboardEvent) => {
    ...
    const shortcutMenus = Array.from(
      panelDoc.querySelectorAll("#paperpilot-shortcut-menu"),
    ) as HTMLDivElement[];
    ...
    chatBox.addEventListener("click", (event: Event) => {
      if (deps.getInlineEditTarget()) {
        const isInsideEdit = (event.target as Element | null)?.closest(
          ".paperpilot-inline-edit-wrapper",
        );
        if (!isInsideEdit) {
      ...
      const copyBtn = target?.closest(
        ".paperpilot-render-copy-btn",
      ) as HTMLButtonElement | null;
      if (copyBtn) {
        event.preventDefault();
        event.stopPropagation();
        const copyable = copyBtn.closest(".paperpilot-copyable") as HTMLElement | null;
        const source = copyable?.dataset.paperpilotCopySource || "";
        if (source) {
  ...
  if (
    ...
    panelDoc.addEventListener("mousedown", (event: Event) => {
      ...
      const modelMenus = Array.from(
        panelDoc.querySelectorAll("#paperpilot-model-menu"),
      ) as HTMLDivElement[];
      const reasoningMenus = Array.from(
        panelDoc.querySelectorAll("#paperpilot-reasoning-menu"),
      ) as HTMLDivElement[];
      const target = event.target as Node | null;
      const retryButtonTarget = deps.isElementNode(target)
        ? (target.closest(".paperpilot-retry-latest") as HTMLButtonElement | null)
        : null;
      const retryModelMenus = Array.from(
        panelDoc.querySelectorAll("#paperpilot-retry-model-menu"),
      ) as HTMLDivElement[];
      const responseMenus = Array.from(
        panelDoc.querySelectorAll("#paperpilot-response-menu"),
      ) as HTMLDivElement[];
      const promptMenus = Array.from(
        panelDoc.querySelectorAll("#paperpilot-prompt-menu"),
      ) as HTMLDivElement[];
      const exportMenus = Array.from(
        panelDoc.querySelectorAll("#paperpilot-export-menu"),
      ) as HTMLDivElement[];
      const slashMenus = Array.from(
        panelDoc.querySelectorAll("#paperpilot-slash-menu"),
      ) as HTMLDivElement[];
      const historyMenus = Array.from(
        panelDoc.querySelectorAll("#paperpilot-history-menu"),
      ) as HTMLDivElement[];
      const historyNewMenus = Array.from(
        panelDoc.querySelectorAll("#paperpilot-history-new-menu"),
      ) as HTMLDivElement[];
      const historyRowMenus = Array.from(
        panelDoc.querySelectorAll("#paperpilot-history-row-menu"),
      ) as HTMLDivElement[];
      const shortcutMenus = Array.from(
        panelDoc.querySelectorAll("#paperpilot-shortcut-menu"),
      ) as HTMLDivElement[];
